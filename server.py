...
def _production_order_row_dict_by_row_num(sheets, row_num: int) -> dict:
    """Read one Production Orders row through column BZ (includes Order Ship * cols)."""
    header_row = _production_orders_header_row(sheets)
    ...
    try:
        row_vals = (
            ...
            .get(
                spreadsheetId=SPREADSHEET_ID,
                range=f"Production Orders!A{row_num}:BZ{row_num}",
            )
            .execute()
            .get("values", [[]])[0]
        )
...
def _qbo_canonical_ship_via_label(raw: str) -> str:
    """
    Map UI / rate-tile labels to QBO Ship Via names (e.g. 'UPS - Ground' → 'UPS Ground').
    Matches printed invoices where the Ship Via dropdown shows 'UPS Ground'.
    """
    s = str(raw or "").strip()
    if not s:
        return "UPS Ground"
    ...
    if "bill only" in low or "(no ups)" in low:
        return "UPS"
    m = re.match(r"^ups\s*[-–—]\s*(.+)$", s, re.I)
    if m:
        tier = re.sub(r"\s+", " ", m.group(1).strip())
        if not tier:
            return "UPS Ground"
        if tier.lower() == "ground":
            return "UPS Ground"
        if tier.lower().startswith("ups "):
            return tier[:31]
        return f"UPS {tier}"[:31]
    if low == "ground":
        return "UPS Ground"
...
def get_or_create_ship_method_ref(name, headers, realm_id, env_override=None):
    ...
    raw = _qbo_canonical_ship_via_label(str(name or "").strip() or "UPS")
    candidates = []
    for c in (
        raw,
        "UPS Ground",
...
def _ensure_qbo_invoice_ship_via(
    ...
    if r2.status_code not in (200, 201):
        ...
        if _qbo_try_full_invoice_update_delivery_overlays(
            headers,
            realm_id,
            iid,
            {"ShipMethodRef": dict(ship_method_ref_payload)},
            ...
            logging.info(
                "QBO Ship Via fix: full invoice POST recovered ShipMethodRef for invoice %s",
                iid,
            )
        return
    try:
    ...
    if _qbo_try_full_invoice_update_delivery_overlays(
        ...
        logging.info(
            "QBO Ship Via fix: full invoice POST recovered ShipMethodRef for invoice %s", iid
        )
...
def _qbo_try_full_invoice_update_shipamt(
    ...
    *,
    ...
    def _apply_delivery_fields(body_dict):
        body_dict["ShipAmt"] = wanted
        if isinstance(ship_method_ref_payload, dict) and str(
            ship_method_ref_payload.get("value") or ""
        ).strip():
            body_dict["ShipMethodRef"] = dict(ship_method_ref_payload)
        tr = str(tracking_num or "").strip()
        if tr:
            body_dict["TrackingNum"] = tr
        sd = str(ship_date or "").strip()
        if sd:
            body_dict["ShipDate"] = sd

    # Attempt 1: minimal strip + sparse=false + ShipAmt (+ Ship Via when provided)
    body = _sanitize_invoice_body_for_full_update(inv)
    body["sparse"] = False
    _apply_delivery_fields(body)
    r2 = _attempt(body)
    if r2.status_code in (200, 201):
        logging.info(
...
def _qbo_append_shipping_sales_line_to_invoice(
    headers,
    realm_id,
    invoice_id,
    ship_amt_r,
    ...
    """Append a Shipping sales line when native ShipAmt / Ship Via cannot be set."""
    try:
        want = round(float(ship_amt_r or 0), 2)
    except (TypeError, ValueError):
        want = 0.0
    if want <= 0:
        return False
    inv, _err = _qbo_get_invoice(headers, realm_id, invoice_id, env_override)
    if not inv:
        return False
    lines = inv.get("Line") or []
    if not isinstance(lines, list):
        lines = []
    for ln in lines:
        if isinstance(ln, dict) and str(ln.get("Description") or "").startswith(
            "Shipping ("
        ):
            return True
    try:
        ...
        logging.warning(
            "QBO append shipping line: could not resolve Shipping item: %s", ex
        )
        return False
    sv = _qbo_canonical_ship_via_label(ship_via_label)
    new_line = {
        "DetailType": "SalesItemLineDetail",
        "Amount": float(want),
        "Description": f"Shipping ({sv})",
        "SalesItemLineDetail": {
            "ItemRef": {"value": str(ship_item["value"])},
            "Qty": 1.0,
            "UnitPrice": float(want),
        },
    }
    merged_lines = list(lines) + [new_line]
    return _qbo_try_full_invoice_update_delivery_overlays(
        headers,
        realm_id,
        invoice_id,
        {"Line": merged_lines},
...
def _qbo_ensure_invoice_shipping_complete(
    headers,
    realm_id,
    invoice_id,
    ship_amt_r,
    ...
    inv = _qbo_log_invoice_shipping_diagnostics(
        "ensure_shipping_start",
        headers,
        realm_id,
        invoice_id,
        env_override,
        wanted_ship_amt=want_amt,
    ...
    has_sm = bool(str(snap.get("ShipMethodRef_value") or "").strip())
    want_sm_val = (
    ...
    if need_amt or need_sm:
        overlays = {}
        if need_amt:
            overlays["ShipAmt"] = float(want_amt)
        if need_sm and isinstance(ship_method_ref_payload, dict):
            overlays["ShipMethodRef"] = dict(ship_method_ref_payload)
        if wanted_track and not str(inv.get("TrackingNum") or "").strip():
            overlays["TrackingNum"] = wanted_track
        if txn_date_str and str(inv.get("ShipDate") or "").strip() != str(
            txn_date_str
        ).strip():
            overlays["ShipDate"] = str(txn_date_str).strip()
        if overlays:
            _qbo_try_full_invoice_update_delivery_overlays(
                headers, realm_id, invoice_id, overlays, env_override
            )
        if need_amt:
            _qbo_try_full_invoice_update_shipamt(
                headers,
                realm_id,
                invoice_id,
                want_amt,
                env_override,
                ship_method_ref_payload=ship_method_ref_payload
                if need_sm
                else None,
                tracking_num=wanted_track or None,
                ship_date=txn_date_str,
            )

    if need_sm and isinstance(ship_method_ref_payload, dict):
        _ensure_qbo_invoice_ship_via(
            headers, realm_id, invoice_id, ship_method_ref_payload, env_override
        )

    inv2 = _qbo_get_invoice(headers, realm_id, invoice_id, env_override)[0]
    if inv2:
        ...
        has_sm2 = bool(str(snap2.get("ShipMethodRef_value") or "").strip())
        if want_amt > 0.001 and cur_amt2 <= 0.001 and not has_line2:
            logging.warning(
                "QBO ensure shipping: native ShipAmt still empty on invoice %s — "
                "appending Shipping sales line $%s",
                invoice_id,
                want_amt,
            )
            _qbo_append_shipping_sales_line_to_invoice(
                headers,
                realm_id,
                invoice_id,
                want_amt,
...
def _qbo_try_full_invoice_update_delivery_overlays(
    headers, realm_id, invoice_id, field_overlays, env_override=None
):
    """
    Full (non-sparse) invoice POST: merge field_overlays onto the latest GET body.

    Intuit often accepts sparse updates but leaves ShipAmt / ShipMethodRef / TrackingNum / ShipDate
    empty in the UI; posting the full invoice with Lines from GET plus delivery fields matches
    community guidance and recovers when per-field sparse passes do not stick.
    """
    iid = str(invoice_id or "").strip()
    if not iid or not isinstance(field_overlays, dict) or not field_overlays:
        return False
    base = get_base_qbo_url(env_override)
    get_url = f"{base}/v3/company/{realm_id}/invoice/{iid}?minorversion={QBO_MINOR_VERSION}"
    r = requests.get(get_url, headers=headers, timeout=40)
    if r.status_code != 200:
        logging.warning(
            "QBO full delivery overlays: GET invoice %s failed HTTP %s: %s",
            iid,
            r.status_code,
            (r.text or "")[:800],
        )
    ...
    if not inv:
        logging.warning(
            "QBO full delivery overlays: GET invoice %s had no Invoice object", iid
        )
        return False
    ...
    merged_overlays = dict(field_overlays)

    body = _sanitize_invoice_body_for_full_update(inv)
    body["sparse"] = False
    body.update(merged_overlays)
    r2 = _attempt(body)
    if r2.status_code in (200, 201):
        logging.info(
            "QBO full delivery overlays OK invoice_id=%s keys=%s (minimal strip)",
            iid,
            list(merged_overlays.keys()),
        )
        return True
    logging.warning(
        "QBO full delivery overlays minimal-strip failed HTTP %s: %s",
        r2.status_code,
        (r2.text or "")[:1000],
    )
    ...
    body2.pop("domain", None)
    body2["sparse"] = False
    body2.update(merged_overlays)
    r3 = _attempt(body2)
    if r3.status_code in (200, 201):
        logging.info(
            "QBO full delivery overlays OK invoice_id=%s keys=%s (light strip)",
            iid,
            list(merged_overlays.keys()),
        )
        return True
    logging.warning(
        "QBO full delivery overlays light-strip failed HTTP %s: %s",
        r3.status_code,
        (r3.text or "")[:1000],
    )
    return False
...
def _qbo_patch_invoice_shipping_and_tracking(
    ...
    if inv2 is not None:
        ...
        # QBO sometimes returns 200 with ShipAmt / ShipMethodRef still empty on a follow-up GET.
        inv_v, _ev = _qbo_get_invoice(headers, realm_id, invoice_id, env_override)
        if inv_v:
            verify_ok = True
            if "ShipAmt" in patch and wanted_amt > 0.001:
                if abs(_curr_amt(inv_v.get("ShipAmt")) - wanted_amt) > 0.02:
                    verify_ok = False
            if "TrackingNum" in patch and wanted_track:
                if str(inv_v.get("TrackingNum") or "").strip() != wanted_track:
                    verify_ok = False
            if "ShipMethodRef" in patch and want_sm_val:
                smv = inv_v.get("ShipMethodRef")
                smd = smv if isinstance(smv, dict) else {}
                if str(smd.get("value") or "").strip() != want_sm_val:
                    verify_ok = False
            if "ShipDate" in patch and txn_date_str:
                if (
                    str(inv_v.get("ShipDate") or "").strip()
                    != str(txn_date_str).strip()
                ):
                    verify_ok = False
            if verify_ok:
                return True
        logging.warning(
            "QBO shipping patch: sparse OK but GET verification failed for invoice %s — "
            "retrying per-field and full POST recovery",
            invoice_id,
        )
    else:
        logging.warning(
    ...
    # Full invoice POST: merge delivery fields onto GET body (recovers when sparse is ignored).
    inv_chk, _ec = _qbo_get_invoice(headers, realm_id, invoice_id, env_override)
    if inv_chk:
        c_amt = _curr_amt(inv_chk.get("ShipAmt"))
        c_sm = (
            inv_chk.get("ShipMethodRef")
            if isinstance(inv_chk.get("ShipMethodRef"), dict)
            else {}
        )
        c_sm_v = str(c_sm.get("value") or "").strip()
        c_tr = str(inv_chk.get("TrackingNum") or "").strip()
        c_sd = str(inv_chk.get("ShipDate") or "").strip()
        want_sd = str(txn_date_str or "").strip()

        need_amt = wanted_amt > 0.001 and c_amt <= 0.001
        need_sm = bool(want_sm_val) and c_sm_v != want_sm_val
        need_tr = bool(wanted_track) and c_tr != wanted_track
        need_sd = bool(want_sd) and c_sd != want_sd

        if need_amt or need_sm or need_tr or need_sd:
            overlays = {}
            if need_amt:
                overlays["ShipAmt"] = float(wanted_amt)
            if need_sm:
                overlays["ShipMethodRef"] = (
                    dict(ship_method_ref_payload)
                    if isinstance(ship_method_ref_payload, dict)
                    else {"value": want_sm_val}
                )
            if need_tr:
                overlays["TrackingNum"] = wanted_track
            if need_sd:
                overlays["ShipDate"] = want_sd
            if _qbo_try_full_invoice_update_delivery_overlays(
                headers, realm_id, invoice_id, overlays, env_override
            ):
                logging.info(
                    "QBO shipping patch: recovered delivery fields via full invoice POST "
                    "invoice_id=%s keys=%s",
                    invoice_id,
                    list(overlays.keys()),
                )
                ...
                if _qbo_try_full_invoice_update_shipamt(
                    ...
                    wanted_amt,
                    env_override,
                    ship_method_ref_payload=overlays.get("ShipMethodRef")
                    if need_sm
                    else ship_method_ref_payload,
                    tracking_num=wanted_track or None,
                    ship_date=txn_date_str,
                ):
                    logging.info(
                        "QBO shipping patch: recovered ShipAmt via full invoice update "
                        "(Amt-only fallback) invoice_id=%s",
                        invoice_id,
                    )
...
def _qbo_invoice_env_label(env_override=None):
    """Return 'production' or 'sandbox' for browser deeplinks (must match OAuth/QBO company)."""
...
def create_invoice_in_quickbooks(
    ...
    ship_via_single = _qbo_canonical_ship_via_label(
        str(shipping_method or "UPS Ground").strip() or "UPS Ground"
    )
    ...
    _qbo_ensure_invoice_shipping_complete(
        headers,
        realm_id,
        inv_id,
        ship_amt_for_fee,
...
def create_consolidated_invoice_in_quickbooks(
    ...
    def _normalize_ship_via_label(raw: str) -> str:
        return _qbo_canonical_ship_via_label(raw)
    ...
    # Ship Via: prefer the exact label from the Ship app (user's chosen rate tile), not service-code mapping.
    ui_ship = str(shipping_method or "").strip()
    code_ship = _ups_service_code_to_ship_via_label(service_code)
    if ui_ship:
        ship_via_label = _normalize_ship_via_label(ui_ship)
    else:
        ship_via_label = _normalize_ship_via_label(
            (code_ship or "UPS Ground").strip()
        )
    logging.info(
        "Consolidated invoice ship-via: service_code=%r shipping_method=%r code_fallback=%r -> ship_via_label=%r",
        service_code,
        shipping_method,
        code_ship,
        ship_via_label,
    ...
    def _fallback_invoice_payload(*, include_header_ship_amt=True):
        ...
        When include_header_ship_amt is True, sends header ShipAmt **and** a Shipping sales line
        (duplicate freight until you remove one in QBO). Intuit often returns 2010 on that combo
        when the ShipMethod entity is missing from the API — use include_header_ship_amt=False
        (freight on the Sales line only) for those companies.
        ...
        # Optional header ShipAmt; when ShipMethod API is dead, duplicate line+header often 2010s.
        if ship_amt_r > 0 and include_header_ship_amt:
    ...
    # ShipMethod query/create returns 4001/500 for this file — native ShipAmt on POST almost always
    # 2010s; always run the shipping-line-before-minimal path without requiring an env flag.
    if qbo_native_ship_off:
        auto_ship_line_on_2010 = True

    used_force_line = False
    force_attempted = False
    res = None
    invoice_payload = None
    freight_line_only_for_native_off = False
    ...
    if (
        ...
        logging.info(
            "QBO invoice 2010: continuing to minimal create + sparse ship/tracking patch "
            "(shipping-line-before-minimal did not apply or did not resolve)"
        )
...
def jobs_for_company():
    ...
    try:
        # Through BZ so Order Ship * columns (incl. ZIP past AZ) are available for the Ship tab.
...
def _product_type_display(prop, needs_front_back=False, is_back=False):
    """Map _productType (and _malletType) to sheet display: Driver, Fairway, Driver Front, Driver Back, etc."""
    pt = (prop("_productType") or "driver").strip().lower()
    mallet = (prop("_malletType") or "").strip().lower()
    name = {
        "driver": "Driver",
        "fairway": "Fairway",
        "hybrid": "Hybrid",
...
def shopify_webhook_orders_create():
    ...
    try:
        ...
        logger.warning("[ShopifyWebhook] Drive not available: %s", e)
        drive = None

    from googleapiclient.http import MediaIoBaseUpload

    def _make_public_safe(drive_svc, file_id):
        try:
...
def process_shipment():
    ...
    try:
        ...
        if not skip_invoice and qbo_invoice_id:
            ...
            resp_payload["qbo_invoice_env"] = _qbo_invoice_env_label(env_override)
