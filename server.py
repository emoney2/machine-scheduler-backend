    ...
    try:
        ...
        vals = svc.get(
            ...
            valueRenderOption="UNFORMATTED_VALUE"
                ...
                # Map Overview columns; keep this list aligned with Overview!A3:K
                headers = ["Order #","Preview","Company Name","Design","Product","Stage","Due Date","Print","Ship Date","Hard Date/Soft Date","Notes"]
                ...
                for r in vals:
                    ...
                    # First try Preview column
                    row["image"] = thumb(row.get("Preview"))
                    jobs.append(row)

                # 🧩 If any job still lacks an image, pull from Production Orders "Image" column by Order #
                try:
                    missing_orders = [j.get("Order #") for j in jobs if not j.get("image")]
                    if missing_orders:
                        svc = get_sheets_service().spreadsheets().values()
                        # Read headers to find "Order #" and "Image" columns dynamically
                        po_hdr = svc.get(
                            spreadsheetId=SPREADSHEET_ID,
                        ...
                        c_order = idx_of("Order #")
                        c_image = idx_of("Image")
                        if c_order >= 0 and c_image >= 0:
                            po_rows = svc.get(
                                spreadsheetId=SPREADSHEET_ID,
                            ...
                            img_map = {}
                            for row in po_rows:
                                row = row + [""] * (max(c_order, c_image) + 1 - len(row))
                                order = row[c_order]
                                image_link = row[c_image]
                                if order and image_link:
                                    img_map[str(order).strip()] = thumb(image_link)

                            for j in jobs:
                                if not j.get("image"):
                                    oid = str(j.get("Order #") or "").strip()
                                    if oid in img_map and img_map[oid]:
                                        j["image"] = img_map[oid]
                except Exception as e:
                    app.logger.warning(f"overview: image enrichment failed: {e}")
